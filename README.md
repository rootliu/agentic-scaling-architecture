...
架构图（`site/figures/`）由 `build-figures.js`（图 1–6，三层架构/agent loop 走查）与 `build-figures-skilltrain.js`（图 7 双子目标 Reward 训练环、图 8 Skill 生命周期，中英 SVG+PNG）生成，`render.sh` 负责 SVG→PDF/PNG 渲染。

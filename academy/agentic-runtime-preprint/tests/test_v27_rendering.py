"""v27 formal-foundations manuscript rendering tests."""
import hashlib
import re
import shutil
import subprocess
...
class V27RenderingTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls._tmp = tempfile.TemporaryDirectory(prefix="agentic-runtime-v27-")
        out = Path(cls._tmp.name) / "v27.pdf"
        subprocess.run(
            [
                sys.executable,
                str(RENDERER),
    ...
    def test_default_build_writes_v27_and_preserves_frozen_releases(self):
        before = {
            version: hashlib.sha256(release(version).read_bytes()).hexdigest().upper()
            for version in FROZEN
        }
        with tempfile.TemporaryDirectory(prefix="agentic-runtime-v27-default-") as tmp:
    ...
    def test_formal_appendix_states_results_and_boundaries(self):
        for term in (
            "Technical Appendix: Information-Theoretic and Discrete Foundations",
            "query-agnostic",
            "Decoder-independent error floor",
            "Minimal Sufficient Representation",
            "Linear-logarithmic separation",
            "Interface indistinguishability",
            "finite alphabets",
            "does not show that the Data Wiki reduces latency or cost",
        ):
            self.assertIn(term, self.text)
        self.assertNotIn("lceil", self.text)
        self.assertNotIn("rceil", self.text)
    ...
    def test_vector_figure_uses_current_hypothesis_labels(self):
    ...
    def test_png_figures_do_not_duplicate_the_responsibility_band(self):
        spec = importlib.util.spec_from_file_location("v27_renderer", RENDERER)

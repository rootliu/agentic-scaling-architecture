"""v29 release rendering, citation closure and inherited claim-boundary checks.

Prior release PDFs stay frozen. Earlier citation-boundary assertions remain
active on the new manuscript; finite contract conformance has its own artifact.
"""
import hashlib
...
FROZEN = {
    "v28": "6E5D3E12E226311219F12189577F88C3BCD757C2B7B6B0EF8140B61C2340F19B",
...
class V29RenderingTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls._tmp = tempfile.TemporaryDirectory(prefix="agentic-runtime-v29-")
        out = Path(cls._tmp.name) / "v29.pdf"
        subprocess.run(
    ...
    def test_default_build_writes_v29_and_preserves_frozen_releases(self):
        before = {v: hashlib.sha256(release(v).read_bytes()).hexdigest().upper()
                  for v in FROZEN}
        with tempfile.TemporaryDirectory(prefix="agentic-runtime-v29-default-") as tmp:
            ...
            expected = (isolated / "output" / "pdf"
                        / "Scalable_Manageable_Agentic_Runtime_Preprint_v29.pdf")

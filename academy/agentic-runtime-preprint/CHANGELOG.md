...
**补齐三处无外部支撑的主张 + IR 缩写消歧。** 起因是审计本地 PDF 库与 bib 的差集：`academy/papers/pdf/` 有 33 份 PDF，其中 16 份未被论文引用。逐篇核验元数据并评估相关性后（记录见 vault `02-2026相关论文清单` §3），确认 3 篇不是"多引一篇更好看"，而是各自对着论文里一处**当时没有任何外部支撑的主张**。37 → 39 页，bib 45 → 48 条。

### 三处补引（每处都带划界，不是无限定背书）

- **§4 harness 演化簇补 MemoHarness（`2607.14159`）**。该段已引 Harness Handbook、GSME、WML、Phantom Guardrails，独缺这篇同簇工作，读起来像漏检。措辞明确：引它作为"harness 状态是受治资产"的证据，**不作为任何具体累积策略安全的证据** —— 后者正是 §8.3 把保留、删除、授权交给治理契约而非写成架构性质的理由。
- **§8.3 决议 2 与 §10 shared-state isolation 补 CoAgent（`2606.15376`）**。v26 已把并发写入改为"后端中立的幂等与排序契约、不假设任何通用单写者设计"，但该决议此前零外部支撑。措辞明确：引它以论证**把排序契约当作声明式接口**是合理的，**而非采纳其任何具体机制**，且本文不提出自己的吞吐或争用主张。§10 处用它说明"未声明的争用不是假想的"，因此该义务靠仪器化而非论证。
- **§6.4 audit 补 AgentForesight（`2605.08715`）**，并显式划界：**在线预测某次运行可能失败，与证明监测器覆盖了声明渠道集中的每个事件，是两个不同问题**。早期预警可以很准而威胁模型覆盖率仍未被测量 —— 这正是六义务要求独立事件清单、而不接受预测器置信度的原因。

### IR 缩写消歧（§8.2）

相邻文献里 IR 更常指 *intermediate **representation***（编译器式产物，由生成器发出、后端消费），本地 PDF 库中的 CADIR（`2608.00891`）即是此义。§8.2 首次定义处新增一句区分：本文的 IR 是 task theme 与其准入源集合之间**版本化、可审计的 relation**，由 Harness 在准入时读取，而不是被编译成产物。沿用本仓库处理 Scaffold（vs workflow scaffold）与 three-layer（vs Jagarin）撞车的先例。

### 引用卫生

三条 bib 条目的标题与**完整作者列表**均取自各 PDF 的 `/Title`、`/Author` 元数据，未据本地文件名转写 —— 本轮审计已证明文件名不可靠（33 份中 8 份标题与原文不符，5 份为实质性错误）。正文 48 处 `\cite` 与 bib 48 条双向闭合，零缺失、零未用。

评估为"可选"的 4 篇（`2606.06003`、`2605.14892`、`2605.29790`、`2606.05023`）与"低优先"的 2 篇未引入；7 篇领域应用（CAD / 化工 / 物理 / 领域 RAG）判定离题。

### 版本与测试

- `PREPRINT_VERSION` 升为 v28；v27 PDF 以 SHA-256 `69101CC2…` 冻结，`test_v27_rendering.py` 缩减为单条冻结校验（与 v27 当初处理 v26 一致）。
- 新增 `test_v28_rendering.py`（10 项）：默认输出 v28 且 v21/v22/v23/v26/v27 校验和不变；三处补引**连同其划界措辞**一起断言（防止后续编辑把有限定的引用悄悄改成无限定背书）；IR 消歧；引用双向闭合；三条新 key 携带核验过的标题与一作；7 张图内嵌；`--title` 仍在。
- 另加 `test_latex_source_stays_ascii`：起草 IR 消歧那句时曾有一个中文字符混入英文句子（`compiler-style编 artifact`），已修，并用该测试守住这类缺陷。
- 全套 **39 项通过**。

### 未变

三条 standing risk 一条未减：架构仍零实测结果、`supported` 不等于证明独立、组织与治理侧最缺证据。与现有编排框架的逐项对比表仍未落地。

## v27 — 2026-08-23

**写时表示的信息论与离散数学基础。** 将 2026-08-22 Obsidian 形式化笔记中与当前论文主张直接相关的最小自足骨架纳入合并稿：正文增加数据基盘到形式化附录的桥接，附录给出假设、命题、证明和不能外推的边界。

### 形式化内容

- 定义有限任务族下的 query-agnostic 确定性写时表示与亏损 `H(Y|Z,Q)`，用条件 Fano 不等式说明固定表示产生的解码器无关错误下界。
- 将写时编码器刻画为源空间划分，证明充分性等价于编码划分细化最小充分划分，并得到 `R_min = H(pi*)` 及查询族扩张的单调下界。
- 给出坐标查询族的线性—对数分离：写时充分表示至少 `k` bit；保留可寻址源数据时，一次读时请求与返回为 `ceil(log2 k)+1` bit；明确两者比较的是不同资源，未把原始存储、索引、授权和时延写成零成本。
- 用接口不可区分性统一描述粗寻址、顺序不变聚合与超预算切片；不把某种检索技术写成普遍优劣结论。

### 论证边界

- 正文 P16 仍是待验证的任务族证据路径假设；形式化附录不替代 held-out 对照实验，也不证明 Data Wiki 的时延、成本或检索质量优势。
- 明确原始保留与稳定身份只是 source-fallback 恢复路径的必要组成，不自动保证可访问、正确、新鲜、授权或可审计。
- 限定结论依赖有限字母表、确定编码器、精确答案和持久化/在线通信分账；随机编码、连续任务、近似答案与全生命周期成本留待扩展。

### 版本与测试

- `PREPRINT_VERSION` 升为 v27；v26 PDF 以 SHA-256 冻结。
- 新增 v27 默认输出、形式化术语、假设边界和历史版本不变性回归测试。

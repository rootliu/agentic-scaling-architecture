...
class FigureGraphic(Flowable):
    ...
    def _draw_overview(self, canvas):
        w, h = self.width, self.height
        draw_wrapped(canvas, "Agentic runtime as a managed memory loop", 16, h - 12, w - 32,
                     "Helvetica-Bold", 8.6, "#0f172a")
        self._pill(canvas, 16, 70, 76, 54, "User intent", "request, policy, output need",
                   "#eff6ff", "#60a5fa")
        self._pill(canvas, 112, 54, 88, 88, "Harness", "interpret\nplan\nexecute\ntrace",
                   "#ffffff", "#2563eb", "#1d4ed8")
        self._pill(canvas, 228, 122, 102, 36, "Tier 3 Theme", "reusable plans",
                   "#ecfdf5", "#10b981", "#047857")
        self._pill(canvas, 228, 73, 102, 36, "Tier 2 Index & Link", "5W1H+Which graph",
                   "#fefce8", "#eab308", "#854d0e")
        self._pill(canvas, 228, 24, 102, 36, "Tier 1 Raw Data", "lakehouse catalog",
                   "#fff7ed", "#f97316", "#9a3412")
        self._pill(canvas, w - 92, 70, 76, 54, "Output", "answer plus audit trace",
                   "#eef2ff", "#6366f1", "#3730a3")
        draw_arrow(canvas, 92, 97, 112, 97, "#2563eb")
        draw_arrow(canvas, 200, 112, 228, 140, "#10b981")
        draw_arrow(canvas, 200, 97, 228, 91, "#ca8a04")
        draw_arrow(canvas, 200, 82, 228, 42, "#f97316")
        draw_arrow(canvas, 330, 140, w - 92, 111, "#10b981")
        draw_arrow(canvas, 330, 91, w - 92, 97, "#ca8a04")
        draw_arrow(canvas, 330, 42, w - 92, 83, "#f97316")
        draw_arrow(canvas, w - 54, 70, 286, 122, "#64748b", dashed=True)
        draw_wrapped(canvas, "trace feedback", w - 144, 142, 96,
                     "Helvetica", 6.2, "#64748b")

    def _draw_stack(self, canvas):
        w, h = self.width, self.height
        draw_wrapped(canvas, "Three-tier memory stack", 18, h - 12, w - 36, "Helvetica-Bold", 8.8, "#0f172a")
        x, layer_w, layer_h = 74, w - 148, 34
        layers = [
            (112, "Tier 3: Data Theme", "experience reuse, plan templates, success signals", "#ecfdf5", "#10b981", "#047857"),
            (70, "Tier 2: Index & Link", "5W1H+Which fact atoms, graph links, provenance", "#fefce8", "#eab308", "#854d0e"),
            (28, "Tier 1: Raw Data", "lakehouse assets, reports, APIs, traces", "#fff7ed", "#f97316", "#9a3412"),
        ]
        for y, title, sub, fill, stroke, color in layers:
            self._pill(canvas, x, y, layer_w, layer_h, title, sub, fill, stroke, color)
        self._pill(canvas, 16, 66, 42, 54, "OPIC", "offline indexing workers", "#e0f2fe", "#38bdf8", "#075985")
        draw_arrow(canvas, 58, 72, x, 45, "#f97316")
        draw_arrow(canvas, 58, 94, x, 88, "#eab308")
        draw_arrow(canvas, 58, 116, x, 129, "#10b981")
        draw_arrow(canvas, x + layer_w + 18, 42, x + layer_w + 18, 124, "#0ea5e9")
        draw_wrapped(canvas, "bottom-up indexing", x + layer_w + 24, 92, 62, "Helvetica", 6.4, "#0369a1", TA_LEFT)
        draw_arrow(canvas, x - 18, 124, x - 18, 42, "#7c3aed")
        draw_wrapped(canvas, "top-down reuse", 18, 40, 48, "Helvetica", 6.4, "#6d28d9")

    def _draw_ontology(self, canvas):
        w, h = self.width, self.height
        draw_wrapped(canvas, "5W1H+Which fact atom", 18, h - 12, w - 36, "Helvetica-Bold", 8.8, "#0f172a")
        cx = w / 2
        center = (cx - 52, 74, 104, 36)
        nodes = [
            ("What", "fact type", cx - 42, 126, 84, 28, "#dbeafe", "#2563eb"),
            ("Who", "actors", 50, 101, 84, 28, "#ecfdf5", "#10b981"),
            ("How", "digest", w - 134, 101, 84, 28, "#f5f3ff", "#8b5cf6"),
            ("Where", "sources", 50, 53, 84, 28, "#fff7ed", "#f97316"),
            ("Why", "cause", w - 134, 53, 84, 28, "#fefce8", "#eab308"),
            ("When", "time", cx - 112, 18, 88, 28, "#f1f5f9", "#64748b"),
            ("Which", "scope + relation", cx + 24, 18, 108, 28, "#fee2e2", "#ef4444"),
        ]

        connectors = [
            ("#93c5fd", (cx, center[1] + center[3]), (cx, 126)),
            ("#a7f3d0", (center[0], center[1] + 27), (134, 115)),
            ("#ddd6fe", (center[0] + center[2], center[1] + 27), (w - 134, 115)),
            ("#fed7aa", (center[0], center[1] + 9), (134, 67)),
            ("#fde68a", (center[0] + center[2], center[1] + 9), (w - 134, 67)),
            ("#cbd5e1", (center[0] + 34, center[1]), (cx - 50, 46)),
            ("#fecaca", (center[0] + 70, center[1]), (cx + 54, 46)),
        ]
        canvas.saveState()
        canvas.setLineWidth(0.9)
        for color, start, end in connectors:
            canvas.setStrokeColor(hx(color))
            canvas.setFillColor(hx(color))
            canvas.line(start[0], start[1], end[0], end[1])
            canvas.circle(end[0], end[1], 1.3, stroke=0, fill=1)
        canvas.restoreState()

        self._pill(canvas, center[0], center[1], center[2], center[3],
                   "Fact atom", "typed, sourced, linkable", "#ffffff", "#334155", "#0f172a")
        for title, sub, x, y, node_w, node_h, fill, stroke in nodes:
            self._pill(canvas, x, y, node_w, node_h, title, sub, fill, stroke, "#0f172a")

    def _draw_double_helix(self, canvas):
        w, h = self.width, self.height
        draw_wrapped(canvas, "Double-helix memory mechanism", 18, h - 12, w - 36, "Helvetica-Bold", 8.8, "#0f172a")
        left, right = 44, w - 44
        mid, amp = 82, 32
        xs = [left + i * (right - left) / 120 for i in range(121)]
        top_points = [(x, mid + amp * math.sin((x - left) / (right - left) * math.pi * 2.2)) for x in xs]
        bot_points = [(x, mid - amp * math.sin((x - left) / (right - left) * math.pi * 2.2)) for x in xs]
        canvas.setLineWidth(1.5)
        canvas.setStrokeColor(hx("#2563eb"))
        for (x1, y1), (x2, y2) in zip(top_points, top_points[1:]):
            canvas.line(x1, y1, x2, y2)
        canvas.setStrokeColor(hx("#10b981"))
        for (x1, y1), (x2, y2) in zip(bot_points, bot_points[1:]):
            canvas.line(x1, y1, x2, y2)
        for idx in [8, 28, 48, 68, 88, 108]:
            x1, y1 = top_points[idx]
            x2, y2 = bot_points[idx]
            canvas.setStrokeColor(hx("#94a3b8"))
            canvas.setLineWidth(0.6)
            canvas.line(x1, y1, x2, y2)
        labels = [
            ("Extract", 58, 126, "#2563eb"),
            ("Link", 132, 38, "#10b981"),
            ("Theme", 202, 126, "#2563eb"),
            ("Plan", 270, 38, "#10b981"),
            ("Validate", 336, 126, "#2563eb"),
        ]
        for label, x, y, color in labels:
            self._pill(canvas, x - 28, y - 12, 56, 24, label, "", "#ffffff", color, color)
        draw_wrapped(canvas, "bottom-up: raw evidence -> facts -> links", 34, 24, 170, "Helvetica", 6.5, "#2563eb", TA_LEFT)
        draw_wrapped(canvas, "top-down: themes -> plans -> salience", w - 208, 24, 174, "Helvetica", 6.5, "#047857", TA_RIGHT)

    def _draw_architecture_flow(self, canvas):
        w, h = self.width, self.height
        draw_wrapped(canvas, "Harness runtime protocol", 18, h - 12, w - 36, "Helvetica-Bold", 8.8, "#0f172a")
        cards = [
            (18, 96, "1", "Intent", "parse request and constraints"),
            (162, 96, "2", "Theme", "warm-start from prior plans"),
            (306, 96, "3", "Plan", "decompose and gate access"),
            (18, 34, "6", "Trace", "log outcome for reuse"),
            (162, 34, "5", "Raw fetch", "catalog-governed evidence"),
            (306, 34, "4", "Index", "search and traverse facts"),
        ]
        for x, y, num, title, sub in cards:
            self._pill(canvas, x, y, 118, 42, f"{num}. {title}", sub, "#ffffff", "#94a3b8")
        draw_arrow(canvas, 136, 117, 162, 117, "#2563eb")
        draw_arrow(canvas, 280, 117, 306, 117, "#2563eb")
        draw_arrow(canvas, 365, 96, 365, 76, "#2563eb")
        draw_arrow(canvas, 306, 55, 280, 55, "#2563eb")
        draw_arrow(canvas, 162, 55, 136, 55, "#2563eb")
        draw_arrow(canvas, 77, 76, 77, 96, "#64748b", dashed=True)

    def _draw_results_bar(self, canvas):
        raise ValueError("The obsolete benchmark figure is not publishable.")

    def _draw_theme_convergence(self, canvas):
        raise ValueError("The obsolete convergence figure is not publishable.")

...
\begin{table}[t]
\centering
\caption{Comparison of Harness responsibilities across production agent and workflow systems. Assessment based on public documentation and cited sources; rows marked \textsuperscript{†} denote concurrent work (July 2026) positioned in Section~4, and the row for this paper denotes design goals, not validated results. The two concurrent entries are scored only on the responsibilities they address: Skillware governs Skill identity and lifecycle, so its contract and audit entries reflect manifest-level provenance rather than per-run replay; GRACE structures a system instruction rather than a Skill registry, so Skill registration does not apply, and its validation is local edit admission rather than policy gating on an activated path.}
...
Skillware\textsuperscript{†} & yes & partial & no & no & partial \\
GRACE\textsuperscript{†} & n/a & partial & partial & no & no \\
...
\hline
P1 & Conditional separability of logical and physical scaling & Any assumption violation breaks separation & Implicit in \S5.3 \\
P15 & Dual-subgoal reward improves attribution and convergence & Dual gate converges no faster than scalar & \S9.8 \\
P16 & Data Wiki summaries are reconstructively sufficient & Reconstruction gap does not shrink across iterations & \S9.9 \\
P17 & IR decouples data sources from output formats & Change forces edits into wiki internal structure & \S9.9 \\
\hline
\end{tabular}
\end{table}
...
The main agent does not execute task tools. Its function is to summarize the outputs of its sub-agents and to decide whether the run may terminate. It performs a semantic join across the returned summaries and applies top-$k$ selection to retain the best-supported evidence per field, then measures the \textit{satisfaction ratio} of the declared output sub-domains: the fraction of sub-domains of $\sigma^{out}$ for which the joined evidence meets the registered acceptance criteria. Writing $\mathrm{sat}(\sigma^{out})$ for that ratio,

\begin{equation}
\mathrm{sat}(\sigma^{out})=\frac{1}{|\mathrm{dom}(\sigma^{out})|}\sum_{d\in\mathrm{dom}(\sigma^{out})} \mathbb{1}\big[\mathrm{join}_k(\text{summaries})_d \text{ meets criteria}_d\big],
\end{equation}

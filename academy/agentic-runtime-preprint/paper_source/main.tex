...
\item \textbf{Effect non-interference.} Effects authorized for one activated path do not alter another path's admission decision, activated path, authorized effects, binding constraints, or postconditions except through declared dependencies.
\item \textbf{Shared-state isolation.} Mutable state shared across runs, Skills, or Scaffolds is partitioned, snapshotted, or synchronized so that an intervention on one axis does not silently change the other axis's interface semantics.
\item \textbf{Resource invariance.} For compatible Scaffold classes inside $\Omega$, resource count or topology does not change preregistered semantic outcomes beyond the declared equivalence or non-inferiority margins.
\item \textbf{Scheduler independence.} Placement, queueing, retries, and scheduling feedback do not alter interface semantics except through ordering and resource dependencies declared in the contract.
\end{enumerate}
...
Violations become detectable, auditable, and attributable through this requirement, which is not a causal generator of independence. P1 is weakened by failure of preregistered semantic equivalence or non-inferiority, a capacity-response departure, or a capability-count $\times$ Scaffold-count interaction beyond its margin while the declared conditions and operating region are held fixed. A non-significant interaction alone does not establish independence.
...
\textbf{Falsification.} P1 is weakened if a compatible Scaffold intervention leaves a semantic equivalence or non-inferiority interval outside its margin, materially changes an interface semantic, produces a capacity-response departure beyond its margin, or yields a capability-count $\times$ Scaffold-count interaction outside its preregistered bound while the six conditions and $\Omega$ are held fixed. Diagnostic-arm failures localize recoupling but do not by themselves show that the base operating region satisfies or violates P1.
...
\textbf{Offline falsification.} The discrimination and attribution part of P15 is weakened if configuration (c) does not improve preregistered discrimination or protected-regression detection over (b), rejects more Pareto-valid proposals beyond its margin, or reports process violations that the blinded phantom-violation oracle disproves.

\textbf{Online falsification.} The convergence and safety part of P15 is weakened if configuration (c) adds no preregistered convergence or rollout-cost benefit over (b), increases protected-component regressions, degrades attribution validity, or rejects enough Pareto-valid proposals that matched multi-seed training stalls.

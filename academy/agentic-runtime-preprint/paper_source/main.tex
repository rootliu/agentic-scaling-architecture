...
\begin{table}[t]
\centering
\caption{Comparison of Harness responsibilities across production agent and workflow systems. Assessment based on public documentation and cited sources; the row for this paper (marked \textsuperscript{†}) denotes design goals, not validated results. The table is restricted to systems that occupy the same design space, namely those that register task capabilities and execute them. Concurrent research whose object of governance differs, such as Skill identity and lifecycle or an evolving system instruction, is positioned in Section~4 rather than scored here, because a per-responsibility rating would compare mechanisms that were never intended to cover the same surface.}

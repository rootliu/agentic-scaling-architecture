...
To ground the operation-closure requirement in current practice, we inspected the public SkillMD-138K snapshot released with the Skillware ontology \cite{fan2026skillware}: 138,133 content-deduplicated \texttt{SKILL.md} records drawn from 20,556 repositories. Two structural facts stand out. First, frontmatter is nearly universal (136,380 records, 98.73\%), but in the records we examined it carries a name and a natural-language description, that is, the goal field $g$, and rarely more. Second, a lexical detector finds an explicit path token into \texttt{scripts/}, \texttt{references/}, or \texttt{assets/} in 32,069 records (approximately 23.2\%); the authors note that this detector misses implicit dependencies, so the figure is a lower bound on packaged artifacts rather than a census. Even read as a lower bound, it indicates that a large share of the corpus is natural-language text with no declared deterministic execution component. The two Skills we read in full illustrate the gap. A tool-wrapper Skill for audio transcription, from a widely used repository, declares its preconditions twice, in the frontmatter \texttt{requires} field and again as a runtime check inside its shell script, yet declares neither a typed output schema $o$ nor a complete effect set $e$: writing the transcript file and transmitting audio to a third-party service are consequences the contract does not name. A governance Skill for host hardening comes closer, specifying a report shape, a set of read-only checks, and a rule that state-changing actions require approval; even so, its output schema and version identity remain prose rather than typed, independently rejectable fields. These are illustrative readings, not measurements of the full corpus, and we do not claim that operation closure is absent everywhere. We claim only that the closure the architecture requires, declared and testable effects behind a typed interface, is not the prevailing convention in the public record, which is why the Harness must enforce it rather than assume it.

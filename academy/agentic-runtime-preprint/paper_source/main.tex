...
Agentic runtimes face a systems question that task benchmarks do not answer: whether independently deployable behavior and physical execution capacity can change on separate schedules without hidden coupling or prohibitive enforcement cost. We state one primary hypothesis, P1: within a declared operating region, changing activated capability should leave the capacity-response relationship non-inferior up to a preregistered interaction margin, while changing compatible Scaffold capacity should preserve capability semantics up to a preregistered non-inferiority margin. A Harness provides logical admission, path construction, policy mediation, and evidence capture; a Scaffold provides physical execution, isolation, and resource control. We propose a cluster-period randomized crossover experiment with balanced order, reset or washout, repeated seeds and failure regimes, and cluster-aware uncertainty. The design measures the capability-by-Scaffold interaction, semantic outcomes, six mechanism obligations, and enforcement overhead against a declared budget. This paper supplies a responsibility model and a falsifiable measurement protocol. It reports no completed runtime implementation, experiment, dataset, or measured result.
...
Let $c_0$ and $c_1$ be preregistered capability configurations and let $s_0$ and $s_1$ be compatible Scaffold capacity configurations. For a scalar component of runtime response, the primary difference-in-differences estimand is
...
For more than two levels, the same target is estimated as the capability-by-Scaffold interaction in a prespecified cluster-aware model. The phrase capability-by-Scaffold interaction estimand refers to this departure of the capability contrast across capacity settings. P1 does not constrain the marginal contrast $R(c,s_1)-R(c,s_0)$. A positive Scaffold main effect on throughput, or a negative main effect on queueing delay, is expected and does not falsify P1.

Semantic stability is tested with preregistered equivalence or non-inferiority margins on $Q(c,s)$ across compatible values of $s$. The capacity-response interaction is tested against a two-sided materiality margin on $\Delta_R$. The enforcement budget $B_E$ is fixed before outcomes are examined and applies to the vector of overhead measures: every mandatory component of $E(c,s)$ must remain within its declared latency, compute, evidence-volume, and monetary limits. A cheap uninstrumented path cannot satisfy this budget because missing control work makes the evidence ineligible.

\textbf{P1.} Within the declared operating region $\Omega$, an admitted capability intervention preserves the capacity-response relationship within the interaction margin, a compatible Scaffold intervention preserves semantic outcomes within their equivalence or non-inferiority margins, and the measured controls remain within $B_E$.

The decision rule has three states.

\begin{itemize}
\item \textbf{supported within Omega}: all six obligations in Section~\ref{sec:obligations} have sufficient instrumentation coverage, their violation intervals satisfy threshold, the design resolves the preregistered margins, the capability-by-Scaffold interaction lies inside its materiality bound, semantic outcomes satisfy their margins, and enforcement overhead satisfies $B_E$;
\item \textbf{falsified within Omega}: coverage and power are sufficient for a decision, but the interaction exceeds its bound, a required semantic margin fails, a measured obligation exceeds its violation threshold, or enforcement overhead exceeds $B_E$; and
\item \textbf{inconclusive}: instrumentation coverage is insufficient, the interaction or semantic interval does not resolve its margin, crossover contamination remains after reset, a predeclared eligibility rule fails, or another required quantity cannot be estimated with the planned cluster-aware uncertainty.
\end{itemize}
...
Autonomic-computing work organizes monitoring, analysis, planning, and execution around shared knowledge; MAPE-K is relevant here as a control-loop vocabulary, not as evidence that the loop is correct \cite{kephart2003autonomic,ibm2006autonomic}. ISO/IEC 25010 supplies a product-quality vocabulary for performance efficiency, reliability, security, maintainability, and portability, but it does not select margins for this runtime \cite{iso25010}. Queueing theory predicts nonlinear response near saturation, and datacenter studies show that shared-resource interference and tail latency can dominate aggregate averages \cite{kleinrock1975queueing,mars2011bubbleup,dean2013tail}. Those results motivate period design, tail metrics, and resource instrumentation.
...
The six conditions below are not architectural virtues awarded by inspection. Each is a measured obligation with five required outputs: instrumentation coverage, observed violations, uncertainty, enforcement cost, and operating-region exclusions. For obligation $j$, coverage is the fraction of eligible events for which the required monitor produced a joinable record. The denominator must come from an independent inventory, such as effector invocation, state-access, scheduler, or resource-controller logs. Self-reported Harness traces cannot establish their own completeness.

Each obligation declares a coverage floor $\gamma_j$ and a violation ceiling $\nu_j$ before collection. The report includes the observed numerator and denominator, a cluster-aware interval for the violation rate, the corresponding component of E(c,s), and every exclusion used to define eligible events. If the denominator is unavailable, coverage falls below $\gamma_j$, or missingness cannot be bounded independently of treatment, the P1 decision is inconclusive.
...
Passing all six thresholds does not prove that no seventh coupling channel exists. It establishes that the preregistered threat model was measured well enough to apply P1 within Omega. Diagnostic injections should deliberately open known channels to confirm that each monitor detects the violation it is assigned to detect.
...
Clusters cross over through all eligible capability and Scaffold configurations. Period assignment is randomized within prespecified cluster and failure-regime blocks. Order balancing uses counterbalanced sequences so each treatment appears comparably early and late and follows every other treatment often enough to estimate carryover. The analysis includes period and sequence terms fixed before outcome inspection.
...
Every transition uses a prespecified reset or washout procedure. It drains or cancels in-flight work, restores mutable data and policy snapshots, clears or versions caches and sessions, resets rate-limit and retry state, reinitializes scheduler history when feasible, and waits through a measured stabilization interval. Sentinel requests verify the reset before the next period is eligible. Residual carryover beyond its preregistered bound makes the affected sequence inconclusive rather than silently adjusting the bound after outcomes are known.
...
The primary R(c,s) analysis estimates the capability-by-Scaffold interaction with cluster and period structure, sequence and failure-regime terms, and prespecified workload strata. Cluster-aware uncertainty uses a mixed-effects model, cluster-robust interval, randomization inference, or a prespecified combination appropriate to the number of clusters. The method, finite-sample correction, and fallback for too few clusters are fixed in the analysis plan. Request-level standard errors are not valid substitutes.

The Q(c,s) analysis estimates semantic contrasts across compatible s at each c and capability contrasts at each s. Binary, count, and continuous outcomes use scales and margins chosen before collection. Every-requirement satisfaction is reported alongside aggregate task scores so that one strong subtask cannot hide a required failure.

The E(c,s) analysis compares enforcement-on periods with a safe replay, shadow, or component-ablation baseline that preserves the same workload and does not expose real effectors to unmediated traffic. It reports total and component overhead, including admission, mediation, isolation, reset, and evidence processing. The budget decision uses confidence bounds, not point estimates alone.

Power is designed around the interaction margin, not the larger and easier Scaffold main effect. The plan states clusters, periods, observations per period, assumed intracluster correlation, carryover allowance, minimum detectable interaction, semantic non-inferiority margins, and expected attrition from pre-outcome exclusions. If the achieved interval cannot distinguish the interaction bound, the result is inconclusive even when the point estimate is near zero.

...
- 本架构 Harness 的核心能力之一就是"**大模型调用产生新的程序工具**"（[[00-讨论记录与原始构想]] C3，用户原话）。
...
- Tool Forge 的 capsule 含 **capability contract** —— 与 [[01-C5-双扩展解耦形式化与命题]] 命题 P2"Harness 作为显式契约层"概念同源。
- 可引用其 capsule 结构作为 Harness 契约接口的**具体化候选**（本架构此前留白）。

### 3. Router 的 intent-scoped 加载 → 直接缓解 P1 风险点
- [[01]] P1 的最大风险：加 Skills 会撑大 context、间接拖垮 throughput。
...
- [ ] 把 capsule 结构纳入 [[01]] 的 H 契约接口定义
- [ ] 把"intent-scoped 按需加载"写入 [[01]] P1 风险缓解

...
> **v0.2 对齐**：本版把用例走查显式对齐第四维（并行度/局部性，见 [[09-并行度与局部性协同设计]]）——规划期先 **dry-run 探测每个 sub-goal 的工具集闭包 + Scaffold 环境范围（P13/N7）**，据此按 **工具集不重合（M1/M2）** 切成可并行子 agent；子 agent 各自 **冻结稳定工具集/记忆前缀（P11）**、按 **seed 亲和派生（M4）**、仅沿 **summarized context（M3）** 松耦合传递。这把第一性根因（[[09-并行度与局部性协同设计|§0.5]]：agent 唯一能动的是组织 context → 前缀冻结 + 尾部变化）落到一次真实运行上。
...
- **[02b–02c/13b] 规划期 dry-run 切分 + seed 亲和**：先探测工具集闭包/环境范围（**P13/N7**）再按工具集不重合切并行子 agent（**M1/M2**），同工具集同 seed 共置（**M4**）→ 重合度 ρ↓、前缀命中 H↑、并行度 k_max↑，吞吐逼近 1/s（**P10**）。这正是 [[09-并行度与局部性协同设计|§0.5]] 第一性根因（组织 context = 前缀冻结 + 尾部变化）的一次实例化：稳定工具集/记忆入前缀并冻结（**P11**），跨子 agent 仅传摘要（**M3**）。
...
4. **并行度可设计**：多标的/多周期子 agent 工具集不重合、同 seed 共置，前缀冻结（kronos capsule 入前缀）→ H↑、k_max↑，吞吐逼近 1/s（P10–P13，见 [[09-并行度与局部性协同设计]]）。

> SVG 数据流图见 site `dry-run-{zh,en}.html` 的 FinTech 选项卡。

---

*创建：2026-06-29 | v0.2 对齐并行度/局部性：2026-07-11 | 配套 [[论文草稿-三层可扩展Agentic-Runtime-综合v0.2]] §4.4.1 / §5 · [[09-并行度与局部性协同设计]]（P10–P13 / M1–M4 / §0.5 机制根因）*

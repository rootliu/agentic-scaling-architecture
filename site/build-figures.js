...
const D2 = {
  en:{
    ...
    l2:{name:"L2 · Harness",note:"CapabilityCapsule — 8 tool kinds under one MCP-grounded interface",
      colA:["capsule: intent · I/O contract · invariants","tool routing O1 (intent-scoped, top-k)","T1 standard · T2 model-tool · T4 data-query","code generation vs T8 ephemeral vs tool synthesis"],
      colB:["T3 crystallized skill (code-loop, N7)","T5 meta-tool M: compact · writeback (LLM-invoked)","T7 human-in-the-loop (elicitation)","reward / verifier + policy gate + audit (N0)"]},
    l1:{name:"L1 · Scaffold",note:"ExecutionSpec — isolation & resources",
      colA:["isolation: microVM · container · wasm","bash / process / filesystem (T6)","subagent launch / fork = +X (new CPU)"],
      colB:["SSO / identity: OIDC · SAML · SCIM · RBAC","cloud: AWS/GCP/Azure · KMS · S3 · PrivateLink","serving (KV cache · batching) · lifecycle · obs."]},
    d:{name:"Data Subsystem 𝒟",items:["D₁ DataSourceCard — source · auth · quota · NFR","D₂ SemanticSummary — schema* · relations · themes","D₃ DataUsageSkill — sources · join plan · runs","D₄ Lifetime — freshness · retention · budget","Ω  Workspace — RunTrace · artifacts · LLM Wiki"]},
  },
  zh:{
    ...
    l2:{name:"L2 · Harness",note:"CapabilityCapsule — 同一 MCP 底座下的 8 类工具",
      colA:["capsule：intent · I/O 契约 · invariants","工具路由 O1（intent-scoped, top-k）","T1 标准 · T2 模型工具 · T4 数据查询","code generation vs T8 即时生成 vs tool synthesis"],
      colB:["T3 结晶 skill（code-loop, N7）","T5 meta-tool M：compact · writeback（LLM 可调用）","T7 human-in-the-loop（elicitation）","reward / verifier + policy gate + 审计（N0）"]},
    l1:{name:"L1 · Scaffold",note:"ExecutionSpec — 隔离与资源",
      colA:["隔离：microVM · container · wasm","bash / process / filesystem（T6）","subagent 启动 / fork = +X（占新 CPU）"],
      colB:["SSO / 身份：OIDC · SAML · SCIM · RBAC","云：AWS/GCP/Azure · KMS · S3 · PrivateLink","serving（KV cache · batching）· lifecycle · 观测"]},
    d:{name:"数据子系统 𝒟",items:["D₁ DataSourceCard — source · auth · quota · NFR","D₂ SemanticSummary — schema* · relations · themes","D₃ DataUsageSkill — sources · join plan · runs","D₄ Lifetime — freshness · retention · budget","Ω  工作区 — RunTrace · artifacts · LLM Wiki"]},
  }
};
function fig2(t){
  const LX=56, LW=740;
...
const D4 = {
  en:{
    ...
    reuse:"reward-gated → crystallize",
    ...
    subH:"Deriving a sub-agent = three questions:",
    sub3:[["WHERE / resources","LAUNCH = Scaffold fork — new CPU / isolation (+X)"],
          ["WHAT it runs","existing tools · or a T8 generated program"],
          ["WIRING","N6 code-loop dispatches + dependencies"]],
  },
  zh:{
    ...
    subH:"派生 sub-agent = 三问：",
    sub3:[["在哪 / 资源","启动 = Scaffold fork —— 新 CPU / 隔离（+X）"],
          ["跑什么","已有工具 · 或 T8 现编程序"],
          ["接线","N6 code-loop 派发 + 依赖"]],
  }
};
function fig4(t){
  let s=frame(t.title,t.sub);
  ...
  const bw=410, bh=66;
  const L1=168, L2=272, L3=376;            // left authoring steps
  ...
  // right: n4 sub-agents (expanded with 3-question decomposition inline), then n5 verify
  const R4=168, n4h = t.sub3 ? 152 : bh;
  s+=rect(RX+30,R4,bw,n4h,{fill:C.scafFill,stroke:C.scafStroke,sw:1.3,rx:12,filter:false});
  s+=tline(RX+48,R4+26,t.n4[0],{size:13.5,weight:800,fill:C.scafText});
  s+=leftLines(RX+48,R4+46,wrap(t.n4[1],bw-40,10.6),{size:10.6,fill:C.scafText,lh:13});
  if(t.sub3){
    s+=tline(RX+48,R4+74,t.subH,{size:10.6,weight:800,fill:C.ink});
    const pals=[C.scafStroke,C.harnStroke,C.dataStroke];
    t.sub3.forEach((q,i)=>{
      const yy=R4+92+i*17;
      s+=`<circle cx="${RX+52}" cy="${yy-3}" r="4" fill="${pals[i]}"/>`;
      s+=tline(RX+62,yy,q[0]+"：",{size:10,weight:800,fill:pals[i]});
      const lead=(q[0].length)*(q[0].charCodeAt(0)>0x2e7f?11:6.2)+14;
      s+=tline(RX+62+lead,yy,q[1],{size:10,fill:C.mut});
    });
  }
  const R5=R4+n4h+34;
  s+=box(RX+30,R5,"h",t.n5,true);
  s+=vArrow(RX+30+bw/2,R4+n4h,R5,{stroke:C.scafStroke,head:"o"});
  // seam arrow: generate code (L3) --> sub-agents (R4)  = probabilistic->deterministic handoff
  ...
  // loopback verify -> plan (dashed, up the center gap x=620 then across the top)
  s+=elbow([[RX+30,R5+bh/2],[620,R5+bh/2],[620,BANDTOP-14],[LX+30+bw/2,BANDTOP-14],[LX+30+bw/2,L1]],{stroke:C.scafStroke,sw:1.5,head:"a",dash:"6 5"});
  s+=tline(300,BANDTOP-20,t.loopback,{size:10.5,weight:700,fill:C.scafStroke});
...
/* ---------- FIGURE 6 : preset workflow vs LLM-driven dynamic orchestration ---------- */
const D6 = {
  en:{
    title:"Preset Workflow vs. LLM-Driven Dynamic Orchestration",
    sub:"Both call it a “plan”. But one is a design-time configuration; the other is a runtime artifact generated by the LLM.",
    lh:"Preset workflow", lt:"dify · n8n · template agents",
    rh:"LLM-driven proactive agent", rt:"this architecture",
    lsteps:[["Node A","tool bound at design time"],["Node B","fixed next step"],["Node C","fixed output"]],
    lnote:"Fixed flowchart · tools bound to nodes · one path · humans edit the graph to change it.",
    rlib:"Harness tool library (T1–T8)",
    rflow:[["NL intent","user goal in natural language"],
           ["LLM selects + reflects","pick tools from the library, re-compose; build several candidate combos"],
           ["write a new tool if missing","T8 generated code formalizes the combo"],
           ["orchestration code (N6)","the run-specific plan — deterministic to execute, auditable"]],
    rnote:"Plan generated at runtime · tools chosen from the library · many candidates · a new tool is written when one is missing.",
    verdict:"Design-time config  ⟶  Runtime artifact.  “preset agent”  vs  “proactive agent”.",
  },
  zh:{
    title:"预设 workflow  vs  LLM 驱动的动态编排",
    sub:"两者都叫「plan」。但一个是设计期的配置，另一个是 LLM 运行期生成的产物。",
    lh:"预设 workflow", lt:"dify · n8n · 模板式 agent",
    rh:"LLM 驱动的主动式 agent", rt:"本架构",
    lsteps:[["节点 A","设计期绑定工具"],["节点 B","固定的下一步"],["节点 C","固定的输出"]],
    lnote:"固定流程图 · 工具绑在节点上 · 单一路径 · 要改就得人工改图。",
    rlib:"Harness 工具库（T1–T8）",
    rflow:[["自然语言意图","用户目标（NL）"],
           ["LLM 选择 + 反思","从库里选工具、重组；同时建多种候选组合"],
           ["缺工具就现编","T8 生成代码规范该组合"],
           ["编排代码（N6）","这次运行专属的 plan —— 执行确定、可审计"]],
    rnote:"plan 运行期生成 · 工具从库里选 · 多候选 · 缺了就现编新工具。",
    verdict:"设计期配置  ⟶  运行期生成物。  「预设 agent」 vs 「主动式 agent」。",
  }
};
function fig6(t){
  let s=frame(t.title,t.sub);
  ...
  t.rflow.forEach((n,i)=>{
    ...
    s+=tline(rx+16,ry[i]+23,n[0],{size:12.5,weight:800,fill:rpal[i]});
    s+=leftLines(rx+16,ry[i]+40,wrap(n[1],rw-30,10.2),{size:10.2,fill:C.mut,lh:12});
  });
  for(let i=0;i<3;i++) s+=vArrow(rx+rw/2,ry[i]+rh2,ry[i+1],{stroke:C.harnStroke,sw:1.9});
  // fan-out hint on step 2 (LLM selects) — small branch marks
  s+=tline(rx+rw-70,ry[1]+23,"×N",{size:11,weight:800,fill:C.harnText,mono:true});
  s+=rect(RX+22,TOP+PANH-70,RW-44,50,{fill:"#efe7fb",stroke:C.harnStroke,sw:1,rx:9,filter:false});
  s+=leftLines(RX+36,TOP+PANH-46,wrap(t.rnote,RW-70,11),{size:11,fill:C.harnText,lh:15});
  // center divider arrow: design-time -> runtime
  s+=hArrow(LX+LW,RX,TOP+PANH/2,{stroke:C.ink,sw:2,head:"k"});
  // bottom verdict band
  const BY=TOP+PANH+20;
  s+=rect(LX,BY,RX+RW-LX,44,{fill:"#fbfaf6",stroke:C.neutStroke,sw:1.3,rx:12,dash:"6 5",filter:false});
  s+=center((LX+RX+RW)/2,BY+24,[t.verdict],{size:13,weight:800,fill:C.ink});
  return svgWrap(s);
}

/* ---------- emit ---------- */
const FIGS={f1:fig1,f2:fig2,f3:fig3,f4:fig4,f5:fig5,f6:fig6}, DICT={f1:D1,f2:D2,f3:D3,f4:D4,f5:D5,f6:D6};
...
const PAGE = {
  en:{
    ...
    secs:[
      ...
      {n:"4", h:"Code as the contract — the real engine of the Harness", fig:"f4",
       p:[ "Under the hood the Harness is not just a bag of tools; it is a <b>managed runtime whose contract is code</b>. From a Skill’s requirements the LLM drafts a <b>plan</b> — sub-goals and their order, written in natural language and governed by a PRD (the skill’s acceptance spec). For each sub-goal several tools may apply, so the LLM does a <b>probabilistic fan-out</b>: it writes <b>orchestration code</b> for competing candidates, dispatches them to sub-agents (whose dependencies are themselves expressed in that generated code), then <b>verifies and selects</b> the best. That is the agent loop.",
            "Crucially, this plan is <b>not a preset workflow</b> (as in dify / n8n or template-driven agents) — no fixed flowchart, no tools bound to nodes at design time. It is a <b>proactive, LLM-driven agent</b>: at runtime the LLM selects tools from the harness library (T1–T8), reflects and re-composes them, can build several candidate combinations at once, and — when a capability is missing — writes a new tool (T8) to formalize how that combination runs. The plan is a <i>runtime artifact</i>, not a design-time configuration.",
      ...
      {n:"6", h:"Not a preset workflow — a proactive, LLM-driven agent", fig:"f6",
       p:[ "It is worth stating what this architecture is <i>not</i>. In tools like dify / n8n or template-driven agents, the “plan” is a <b>design-time configuration</b>: a fixed flowchart whose nodes have tools bound to them; to change behavior a human edits the graph.",
            "Here the plan is a <b>runtime artifact</b>. Given a goal in natural language, the LLM selects tools from the harness library (T1–T8), reflects and re-composes them, builds several candidate combinations at once, and — when a capability is missing — writes a new tool (T8) to formalize how the combination runs. The result is orchestration code specific to this run (N6): probabilistic to author, deterministic and auditable to execute. That is the line between a <b>preset agent</b> and a <b>proactive agent</b>." ]},
    ],
    ...
    figcaps:{f1:"Figure 1 — the three layers and what each is for.",f2:"Figure 2 — the spec list inside each layer.",f3:"Figure 3 — the agent loop, step by step.",f4:"Figure 4 — code as the contract: probabilistic authoring, deterministic execution.",f5:"Figure 5 — the eight kinds of tool under one capsule interface.",f6:"Figure 6 — preset workflow vs. LLM-driven dynamic orchestration."},
  ...
  zh:{
    ...
    secs:[
      ...
      {n:"4", h:"Code 即契约 —— Harness 真正的引擎", fig:"f4",
       p:[ "深入看，Harness 不是「一堆工具」，而是一个<b>以 code 为契约的可管理运行时</b>。LLM 依 Skill 的要求产出 <b>plan</b>——子目标及其顺序，主体是自然语言、验收由 PRD（skill 的验收规约）控制。每个子目标可能有多个工具适用，于是 LLM 做<b>概率性 fan-out</b>：为竞争候选<b>生成编排代码</b>、派给子 agent（子 agent 之间的依赖也写在这段生成代码里），再<b>verify 并筛选</b>出最优。这就是 agent 循环。",
            "关键在于：这个 plan <b>不是预设 workflow</b>（不像 dify / n8n 或模板式 agent）——没有设计期画死的流程图，也没有绑定在节点上的固定工具。它是<b>主动式、LLM 驱动的 agent</b>：运行期 LLM 从 harness 工具库（T1–T8）里选工具、反思并重新组合，可同时建多种候选组合；当缺能力时就<b>现编一个新工具（T8）</b>去规范该组合的执行。plan 是<i>运行期的生成物</i>，不是设计期的配置。",
      ...
      {n:"6", h:"不是预设 workflow —— 而是主动式、LLM 驱动的 agent", fig:"f6",
       p:[ "有必要说清本架构<i>不是</i>什么。在 dify / n8n 或模板式 agent 里，「plan」是<b>设计期的配置</b>：一张固定流程图，节点上绑死了工具；要改行为就得人工改图。",
            "而这里的 plan 是<b>运行期的生成物</b>。给一个自然语言目标，LLM 从 harness 工具库（T1–T8）里选工具、反思并重新组合，同时建多种候选组合；当缺能力时就<b>现编一个新工具（T8）</b>去规范该组合怎么跑。产物是这次运行专属的编排代码（N6）：<b>概率性地写、确定且可审计地执行</b>。这就是<b>预设 agent</b> 与<b>主动式 agent</b> 的分界。" ]},
    ],
    claimsH:"可证伪的内核",
    claims:[
    ...
    figcaps:{f1:"图 1 — 三个层及各自的用途。",f2:"图 2 — 每一层里的 spec 清单。",f3:"图 3 — agent loop 的分步走查。",f4:"图 4 — code 即契约：概率生成、确定执行。",f5:"图 5 — 八种工具，同一 Capsule 接口。",f6:"图 6 — 预设 workflow vs LLM 驱动的动态编排。"},
